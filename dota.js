...
const dotaBot = (bot) => {
    ...
    const createReply = (name, match) => {
        ...
        return `玩家${name}最近在${new Date(match.start_time*1000).toLocaleString('zh-Hans-CN', { timeZone: 'America/Vancouver' })}进行了一场比赛，激战了${Math.ceil(match.duration/60)}分钟，终于，他${win?'赢':'输'}了！他玩的是${heroMap.get(match.hero_id)}，他杀了${match.kills}个人，死了${match.deaths}次，助攻了${match.assists}次，KDA为${((match.kills+match.assists)/ (match.deaths===0?1:match.deaths) ).toFixed(2)}，真的是太${((match.kills+match.assists)/match.deaths)>2?'屌':'菜'}了！他的GPM是${match.gold_per_min}，XPM是${match.xp_per_min}，总计造成了${match.hero_damage}点伤害，（wy赞助）经济输出比为${(match.hero_damage/((match.duration/60)*match.gold_per_min)).toFixed(2)}。GGWP！`
    }
